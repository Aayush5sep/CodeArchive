...
async def contest_loop():
	...
	target_channel = bot.get_channel(896310809888714772)
	await target_channel.send('This loop is executed')
	...
	for contests in db['all_contests']:
		if(cday==db[contests]['day'] and cmonth==db[contests]['month'] and (chour==(db[contests]['hour']-1) or chour==db[contests]['hour'] ) ):
			...
			await target_channel.send(embed=clist)
		elif(cday==db[contests]['day'] and cmonth==db[contests]['month'] and chour==(db[contests]['hour']+1)):
			del db[contests]
